...
public class QueuedHostedService : BackgroundService
{
	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		...
		if (incompleteTasks.FirstOrDefault() is { } taskToQueue) await _taskQueue.QueueAsync(taskToQueue);
		foreach (var taskToCancel in incompleteTasks) taskToCancel.Status = CacheTaskStatus.Cancelled;
