...
public static class AssemblyHelpers
{
    ...
    public static (string BaseTypeName, int InheritorCount)[] GetTypesWithInheritors()
    {
        ...
        var assemblyClassTypes = Assembly.GetAssembly(typeof(AssemblyHelpers))!.DefinedTypes.Where(p => p.IsClass);

        var targetNamespace = Assembly.GetAssembly(typeof(AssemblyHelpers))?.GetName().Name;
        var baseTypes = assemblyClassTypes.Select(GetBaseType).Where(p => p?.Namespace?.Equals(targetNamespace) ?? false).Distinct();

        var typesWithInheritors = new (string, int)[baseTypes.Count()];
        int index = 0;
        foreach (var baseType in baseTypes)
        {
            var baseTypeName = baseType!.Name;
            var inheritorCount = Assembly.GetAssembly(typeof(AssemblyHelpers))!.DefinedTypes.Where(p => p.IsAbstract == false && p.IsSubclassOf(baseType)).Count();
            typesWithInheritors[index++] = (baseTypeName, inheritorCount);
        }
